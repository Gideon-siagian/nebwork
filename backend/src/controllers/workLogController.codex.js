...
const canEditWorklog = (worklog, userId) => {
  ...
  return (worklog.collaborators || []).some((collaboratorId) => String(collaboratorId) === String(userId));
};
...
const canViewWorklog = (worklog, user) => {
  ...
  if ((worklog.collaborators || []).some((collaboratorId) => String(collaboratorId) === String(user._id))) {
    return true;
  }
...
const resolveCollaboratorTargets = async ({ email, username, team }) => {
  if (email) {
  ...
  if (username) {
    ...
    const exactEmailPrefix = new RegExp(`^${escapeRegExp(normalized)}@`, "i");
    const users = await User.find({
      $or: [
        { email: exactEmailPrefix },
  ...
  if (team) {
    return User.find({ division: String(team).trim() }).lean();
  }
...
exports.addCollaborator = async (req, res) => {
  try {
    ...
    if (!candidates.length) {
      return res.status(404).json({ message: "No matching users found for this invitation" });
    }
    ...
    candidates.forEach((candidate) => {
      ...
      if (!log.collaborators.some((entry) => String(entry) === candidateId)) {
        log.collaborators.push(candidate._id);
      }
    ...
    const serialized = await getSerializedWorklogById(log._id);
    res.json({
      message: addedUserIds.length > 0 ? "Collaborators updated successfully" : "No new collaborators were added",

...
import { Copy, FileText, Settings2, Users2, Video, History } from "lucide-react";
...
export default function NebworkEditor() {
  ...
  const saveWorklog = async (nextStatus = status, options = {}) => {
    ...
    try {
      ...
      if (options.versionMessage) {
        payload.versionMessage = options.versionMessage;
      }
  ...
  const fetchVersions = async () => {
    ...
    try {
      const response = await fetch(WORKLOG_ENDPOINTS.VERSIONS(worklogId), {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    ...
    >
      <Dialog open={saveMessageOpen} onOpenChange={setSaveMessageOpen}>
        <DialogContent className="max-w-md rounded-[28px] border-border/60 bg-[#fbfdff] p-0">
          <DialogHeader className="border-b border-border/60 px-6 py-5">
            <DialogTitle className="font-display text-2xl text-slate-900">Catatan Versi</DialogTitle>
            <DialogDescription>
              Tuliskan pesan singkat tentang apa yang Anda ubah pada versi ini, agar tampil di riwayat versi.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5 px-6 py-6">
            <div className="space-y-2">
              <Input
                ...
                placeholder="Misal: Update draft konten bagian 1"
                className="h-11 rounded-2xl bg-white"
                autoFocus
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    setSaveMessageOpen(false);
                    saveWorklog("draft", { versionMessage });
                  }
                }}
              />
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" className="rounded-2xl" onClick={() => setSaveMessageOpen(false)}>
                Batal
              </Button>
              <Button
                disabled={isSaving}
      ...
      <Dialog open={versionsOpen} onOpenChange={setVersionsOpen}>
        <DialogContent className="max-w-2xl rounded-[28px] border-border/60 bg-[#fbfdff] p-0 max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader className="border-b border-border/60 px-6 py-5 shrink-0">
            <DialogTitle className="font-display text-2xl text-slate-900">Version history</DialogTitle>
            <DialogDescription>
              Semua riwayat perubahan pada worklog ini tersimpan di sini layaknya jejak kolaborasi.
            </DialogDescription>
          </DialogHeader>
          <div className="flex-1 overflow-y-auto px-6 py-6 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
            {isLoadingVersions ? (
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-sm font-medium text-slate-900">Belum ada riwayat versi</p>
                <p className="text-sm text-slate-500">Perubahan akan didokumentasikan saat worklog disimpan kembali.</p>
              </div>
            ) : (
              <div className="relative border-l-2 border-slate-100 ml-4 space-y-8 pb-4">
                {versions.map((v, index) => (
                  <div key={v._id} className="relative pl-6">
                    {/* Timeline dot */}
                    <div className="absolute left-[-9px] top-1 flex h-4 w-4 items-center justify-center rounded-full border-2 border-white bg-blue-500 shadow-sm">
                    ...
                    <div className="flex items-center justify-between gap-4 mb-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-slate-900">{v.user?.name || "Unknown user"}</p>
                          <p className="text-[11px] font-medium text-slate-400">
                            {new Date(v.datetime).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" })}
                          </p>
                        </div>
                      </div>
                      
                      {index === 0 && (
                        <Badge variant="secondary" className="bg-blue-50 text-blue-700 hover:bg-blue-50 ring-1 ring-blue-600/10">
                          Current version
                        </Badge>
                      )}
                    </div>
                    
                    <div className="rounded-2xl border border-slate-100 bg-[#fcfdfe] p-3.5 shadow-sm">
                      <div className="flex gap-2">
                        <History className="mt-0.5 h-4 w-4 shrink-0 text-slate-400" />
                        <div>
                          <p className="text-sm text-slate-700 leading-relaxed font-medium">{v.message}</p>
                          
                          {v.changedFields && v.changedFields.length > 0 && (
                            <div className="mt-2.5 flex flex-wrap gap-1.5">
                              <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider mr-1 py-0.5">Updated:</span>
                              {v.changedFields.map(f => (
                                <span key={f.fieldName} className="inline-flex items-center rounded-md bg-slate-100 px-2 py-0.5 text-[11px] font-medium text-slate-600">
                                  {f.fieldName}
                                </span>
                              ))}
                            </div>
          ...
          <CardContent className="space-y-5 p-5">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" className="rounded-2xl" onClick={fetchVersions} disabled={!worklogId}>

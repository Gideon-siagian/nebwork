...
export default function NebworkEditor() {
  ...
  }, [content, title, tags, project, department, privacyLevel, summary, collaborators, hasUnsavedChanges, status]);
  ...
  const handleInvite = async (payload) => {
    ...
    try {
      ...
      setFeedback("Collaborator updated.");
      setEmailInvite("");
      setUsernameInvite("");
      setTeamInvite("");
    } catch (inviteError) {
  ...
  return (
    <AppShell
      ...
      actions={
        <>
          <Button variant="outline" className="rounded-2xl" onClick={() => saveWorklog("draft")}>
            Save draft
          </Button>
          <Button
            disabled={isSaving}
      ...
      <Dialog open={inviteOpen} onOpenChange={setInviteOpen}>
        <DialogContent className="max-w-2xl rounded-[28px] border-border/60 bg-[#fbfdff] p-0">
          <DialogHeader className="border-b border-border/60 px-6 py-5">
            ...
            <DialogDescription>
              Tambahkan karyawan lain agar bisa menulis worklog yang sama.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-5 px-6 py-6">
            ...
            <div className="grid gap-4">
              ...
              <div className="space-y-2">
                <p className="text-sm font-medium text-slate-900">Invite by team</p>
                <div className="flex gap-2">
                  <Input value={teamInvite} onChange={(event) => setTeamInvite(event.target.value)} className="h-11 rounded-2xl bg-white" placeholder="Finance Ops - Payroll Squad" />
                  <Button disabled={isInviting || !teamInvite.trim()} className="rounded-2xl" onClick={() => handleInvite({ team: teamInvite.trim() })}>
                    Invite
                  </Button>
                </div>
              </div>
          ...
          <CardContent className="space-y-5 p-5">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" className="rounded-2xl" onClick={handleVideoMeeting} disabled={isMeetingLoading}>
                  <Video className="h-4 w-4" />
                  Video meeting
                </Button>
                <Button className="rounded-2xl bg-[linear-gradient(135deg,#0f172a_0%,#2563eb_100%)] hover:bg-[linear-gradient(135deg,#020617_0%,#1d4ed8_100%)]" onClick={() => setInviteOpen(true)}>
